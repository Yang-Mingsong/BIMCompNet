...
class StructuralDescriptor(nn.Module):
    ...
    def forward(self, corners, normals, neighbor_index):
        # corners三维 （Batch Size, Channe=9, Sequence Length）
        structural_fea1 = self.FRC(corners) # 输出：torch.Size([Batch Size, 64, Sequence Length])
...
class MeshConvolution(nn.Module):
    ...
    def __init__(self, aggregation_method, spatial_in_channel, structural_in_channel, spatial_out_channel, structural_out_channel):
...
class MeshNet(nn.Module):
    def __init__(self, num_classes, mesh_convolution, mask_ratio, dropout, require_fea=False):
        ...
        self.mesh_conv1 = MeshConvolution(mesh_convolution, 64, 131, 256, 256)
        self.mesh_conv2 = MeshConvolution(mesh_convolution, 256, 256, 512, 512)
        self.fusion_mlp = nn.Sequential(
    ...
    def forward(self, data):
        ...
        # Test SpatialDescriptor: torch.Size([2, 64, 20])
